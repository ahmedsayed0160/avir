  ...
  return (
    <div className="font-[Poppins] ">
      ...
      <div className="w-full px-0">
        <div className="text-center flex justify-center items-center py-20 min-h-screen "
          style={{ background: "linear-gradient(to right,#FEF2F2 , #FCE7F3 , #FCE7F3)" }}>
          <div className="w-full lg:w-1/2">
            <span className="px-3 py-2 rounded-full uppercase font-semibold shadow-md"
            ...
            <button className="transition-all duration-300 hover:scale-90 cursor-pointer rounded-full px-6 py-3 text-white capitalize text-base my-8 shadow-lg"
        ...
        <div className="p-10">
          <div className="flex flex-col lg:flex-row justify-evenly items-center">
            <div className="relative w-full lg:w-1/2 mx-auto mb-6 lg:mb-0">
              <img src={heroImg} className="rounded-[30px] shadow w-full" />
            </div>
            <div className="w-full lg:w-1/2 p-10">
              <span className="px-3 py-2 rounded-full uppercase font-semibold"
              ...
              <ul className="my-6">
                {t.survivorStories.features.map((feature, index) => (
                  <li key={index} className="p-3 flex items-center space-x-3">
                    <i className="fa-solid fa-check flex justify-center items-center rounded-full text-center p-2"
                      style={{ color: "#DB2777", backgroundColor: "#FCE7F3" }}></i>
                    <div>
              ...
              <button className="transition-all duration-300 hover:scale-90 cursor-pointer flex items-center justify-center px-6 py-3 rounded-full capitalize text-white font-semibold shadow-lg"
                style={{ backgroundColor: "#DB2777" }}>
          ...
          <div className="w-full lg:w-1/2 p-10">
            ...
            <div className="flex flex-wrap gap-6 py-6">
              {t.treatmentPlans.cards.map((card, index) => (
                <div key={index} className="flex flex-col items-start px-6 py-4 rounded-xl shadow-lg"
                  style={{ flex: "0 0 calc(50% - 0.75rem)", background: "linear-gradient(to bottom,#FAF5FF,#FDF2F8)" }}>
                  <i className={`fa-solid ${card.icon} text-white flex justify-center items-center rounded-lg mb-2 p-4 text-2xl`}
                    style={{ background: card.color }}></i>
                  <h3 className="text-lg py-2 font-bold">{card.title}</h3>
                  <p className="text-[#4B5563]">{card.description}</p>
                </div>
              ))}
            </div>
            <button className="transition-all duration-300 hover:scale-90 cursor-pointer flex items-center justify-center px-6 py-3 rounded-full text-white font-semibold my-3 shadow-lg capitalize"
              style={{ backgroundColor: "#8B5CF6" }}>
              <i className="fa-solid fa-folder-plus mr-2"></i> {t.treatmentPlans.button}
            </button>
          </div>
          <div className="w-full lg:w-1/2 mt-6 lg:mt-0">
            <img src={treatmentImg} className="rounded-xl w-full shadow-lg object-cover" style={{ height: "35rem" }} />
          </div>
        </div>
      </div>
      ...
      <div className="w-full p-10" style={{ background: "linear-gradient(to right,#591c8719 , rgba(0,0,0,0))" }}>
        <div className="flex flex-col lg:flex-row justify-evenly items-center">
          <div className="relative w-full lg:w-1/2 mx-auto mb-6 lg:mb-0">
            <img src={communityImg} className="rounded-[30px] w-full object-contain" style={{ height: "35rem" }} />
          </div>
          <div className="w-full lg:w-1/2 p-10">
            <span className="px-3 py-2 rounded-full uppercase font-semibold"
            ...
            <div className="flex flex-col gap-4 p-6 rounded-xl shadow-lg my-6"
              ...
              {t.community.features.map((feature, index) => (
                <li key={index} className="flex items-center space-x-3">
                  <i className={`fa-solid ${feature.icon} text-white flex justify-center items-center rounded-md p-3`}
                    style={{ background: feature.color }}></i>
                  <div>
            ...
            <div className="flex items-center gap-4 py-6">
              <button className="transition-all duration-300 hover:scale-90 cursor-pointer flex items-center justify-center px-6 py-3 rounded-full text-white font-semibold capitalize shadow-lg"
                style={{ backgroundColor: "#DB2777" }}>
                <i className="fa-solid fa-user-plus mr-2"></i> {t.community.joinButton}
              </button>
              <a href="#" className="capitalize text-black font-semibold link">
